...
import type { Climber, RaceEvent } from '@/themes/everest/types';
import type { FatePlan, Traversal } from '@/themes/everest/decorate';
import { LineWriter } from '@/lib/linewriter';
import { METER_INDEX, nudgeMeter } from '@/themes/everest/meters';
import { NODES, SEGMENTS, distanceLabel, nodeAtOrBelow } from './route';
import type { SpaceCast } from './names';
import {
  SPACE_PHASE_NAMES,
  SPACE_TEMPLATES,
  SPACE_TROUBLES,
...
/**
 * Mission event log. Same assembly discipline as the Everest log — density
 * scaling, max-gap rule, dead crews never speak, fates drawn among eligible
 * crew (never the Commander or the Flight Director) — with the space route
 * and voice.
 */
...
export function buildSpaceEvents(input: BuildEventsInput): RaceEvent[] {
  const {
    rng, core, durationMs, displayTrack, meters, traversals, fate,
    crews, cast, teamNames,
  } = input;
  ...
  ) => writer.render(type, SPACE_TEMPLATES[type], ctx);

  const distAt = (teamIdx: number, tMs: number) =>
    distanceLabel(displayAtTime(displayTrack, teamIdx, tMs));
  ...
  phaseUs.forEach((u, i) => {
    events.push({
      ...
      type: 'phase_change',
      severity: 2,
      text: line('phase_change', { phase: SPACE_PHASE_NAMES[i + 1] }),
    });
  });
  ...
  for (let team = 0; team < n; team++) {
    ...
    for (let i = 1; i < times.length; i++) {
      ...
      const wp = nodeAtOrBelow(row[i] + 0.01);
      const atWp = Math.abs(wp.frac - row[i]) < 0.015;
      if (newMode === 'rest' && atWp && wp.id !== 'PAD') {
        events.push({
          tMs: t, type: 'camp_arrival', teamIdx: team, nodeId: wp.id, severity: 1,
          activity: `Station-keeping at ${wp.label}`,
          text: line('camp_arrival', {
            ...ctxFor(team),
            camp: wp.label,
            alt: distanceLabel(wp.frac),
          }),
        });
        lastEmit = t;
        ...
        events.push({
          ...
          activity: 'Falling back to resupply',
          text: line('descend_rest', ctxFor(team)),
        });
        lastEmit = t;
      } else if (newMode === 'up' && mode === 'rest' && atWp) {
        events.push({
          tMs: t, type: 'camp_depart', teamIdx: team, nodeId: wp.id, severity: 1,
          activity: 'Under burn',
          text: line('camp_depart', { ...ctxFor(team), camp: wp.label }),
        });
        lastEmit = t;
      }
      mode = newMode;
  ...
  // --- Trajectory choices and their consequences ---------------------------
  ...
  for (const tr of traversals) {
    ...
    if (tr.tMs >= core.pushStartMs && tr.segIdx < 5) continue; // catch-up, not a decision
    ...
    if (mover >= 0 && bestGain <= -1) {
      ...
      if (bestGain <= -moveThresh && rng() < 0.6) {
        ...
        events.push({
          ...
          text: line('overtake', {
            ...ctxFor(mover),
            rival: teamNames[rival],
            alt: distAt(mover, tMs),
          }),
        });
      }
    }
    ...
    if (faller >= 0 && worstDrop >= moveThresh && rng() < 0.5 && !wipedAt.has(faller)) {
      ...
      if (recT < core.pushStartMs) {
        events.push({
          ...
          activity: 'Under burn', text: line('recovery', ctxFor(faller)),
        });
      }
    }
  }
      ...
      for (let team = 0; team < n; team++) {
        if (ranks[team] < prevRanks[team] && grid.p[team][gi] < 1) {
          ...
          events.push({
            ...
            text: line('overtake', {
              ...ctxFor(team),
              rival: rival !== undefined ? teamNames[rival] : 'the fleet',
              alt: distAt(team, t),
            }),
          });
          lastOvertakeT = t;
  ...
  // --- Fate: losses, injuries, transfers, dark ships -----------------------
  const fated: Set<number>[] = crews.map(() => new Set<number>());
  const protectedIdx = (teamIdx: number): Set<number> => {
    const s = new Set<number>([0]);
    crews[teamIdx].forEach((c, i) => {
      if (c.role === 'Flight Director') s.add(i);
    });
    return s;
  };
  const pickVictim = (teamIdx: number): number | null => {
    const off = protectedIdx(teamIdx);
    const eligible = crews[teamIdx]
      .map((_, i) => i)
    ...
    if (f.kind === 'fall') {
      ...
      events.push({
        ...
        text: line('climber_fall', {
          ...ctxFor(f.teamIdx),
          climber: crew[climberIdx].name,
          role: crew[climberIdx].role.toLowerCase(),
          alt: distAt(f.teamIdx, f.tMs),
          edge,
        }),
      });
      ...
      events.push({
        ...
        text: line('climber_injured', {
          ...ctxFor(f.teamIdx),
          climber: crew[climberIdx].name,
          trouble: pick(rng, SPACE_TROUBLES),
        }),
      });
  ...
  for (let team = 0; team < n; team++) {
    ...
    for (let i = 1; i < times.length && count < 2; i++) {
      const fuelRow = meters[team][METER_INDEX.O2];
      if (fuelRow[i] - fuelRow[i - 1] > 8) {
        const wp = nodeAtOrBelow(displayTrack.pos[team][i] + 0.01);
        events.push({
          tMs: times[i], type: 'resupply', teamIdx: team, nodeId: wp.id, severity: 1,
          text: line('resupply', { ...ctxFor(team), camp: wp.label }),
        });
        count++;
        i += 10;
      }
  ...
  for (let k = 1; k <= nUpdates; k++) {
    ...
    let cp = null;
    for (const c of core.checkpoints) {
      if (c.tMs <= t) cp = c;
      else break;
    }
    if (!cp) continue;
  ...
  const addAmbient = (tMs: number) => {
    ...
    if (roll < 0.4 && radioTeam !== null) {
      ambient.push({ tMs, type: 'radio', severity: 0, text: line('radio', { camp: pick(rng, NODES).label, team: teamNames[radioTeam], sherpa: cast.director[radioTeam] }) });
    } else if (roll < 0.7) {
      ambient.push({ tMs, type: 'weather', severity: 0, text: line('weather', { weather: pick(rng, SPACE_WEATHER_LINES) }) });
    } else {
      ambient.push({ tMs, type: 'color', severity: 0, text: line('color', { camp: pick(rng, NODES).label, gap: n }) });
    }
  };
  let guard = 0;
